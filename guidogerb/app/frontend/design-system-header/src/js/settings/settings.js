...
const intervalId = setInterval(
  () => {
    ...
    if (
      ...
      || window['design-system-header']?.issetHeaderSettingsCalled
    ) {
...
export function setHeaderSettings(newSettings) {
  ...
  if (window['design-system-header']) {
    // @ts-expect-error window has no index signature
    window['design-system-header'].issetHeaderSettingsCalled = true;
  }
...
export function setFooterSettings(footerSettings) {
  return setHeaderSettings({ ...defaultSettings, ...settingsKeeper.getSettings(), footer: footerSettings });
}
