...
import { renderOfficialWebsite } from '../renderables/guidogerbLogo/renderOfficialWebsite';
import { getHeaderSettings } from '../settings/getHeaderSettings.js';
import { fetchUtahIdUserDataAsync } from '../guidogerbId/guidogerbIdData.js';
...
function loadCssSettings() {
  ...
  const mediaQueriesCssReplaced = mediaQueriesCSS
    .replace(domConstants.MEDIA_SIZE__TABLET_PORTRAIT__PLACEHOLDER, `${getHeaderSettings().mediaSizes.tabletPortrait}px`)
    .replace(domConstants.MEDIA_SIZE__TABLET_LANDSCAPE__PLACEHOLDER, `${getHeaderSettings().mediaSizes.tabletLandscape}px`)
    .replace(domConstants.MEDIA_SIZE__MOBILE__PLACEHOLDER, `${getHeaderSettings().mediaSizes.mobile}px`);
...
/**
 * Based on the current settings, determine in to which DOM element to insert the design system header.
