...
import UtahIdButtonHtml from './html/GuidoGerbIdButton.html?raw';
import UtahIdWrapperHtml from './html/GuidoGerbIdWrapper.html?raw';
...
export function renderUtahIdButton() {
  ...
  // set id user name change immediately to current value so that the button does not flicker
  if (utahIdData) {
...
export function renderUtahIdMenu(shouldAddMenuTitle) {
  ...
  const popupMenuItems = [
    ...
    {
      actionUrl: onProfile ? undefined : { url: guidogerbIdUrls.PROFILE, openInNewTab: true },
      actionFunction: onProfile || undefined,
      title: 'UtahID Profile',
    },
    {
      actionUrl: onSignOut ? undefined : { url: guidogerbIdUrls.SIGN_OUT },
  ...
  const returnMenu = shouldAddMenuTitle ? renderMenuWithTitle(utahIdPopupMenu, 'ID Menu') : utahIdPopupMenu;

  returnMenu.setAttribute('aria-label', 'Id Menu');
      ...
      onClick: (e) => {
        if (!utahIdData?.isDefinitive || !utahIdData?.userInfo?.authenticated) {
          if (onSignIn) {
            ...
            e.preventDefault();
            e.stopPropagation();
            window.location.href = guidogerbIdUrls.SIGN_IN;
          }
        }
      },
    }
