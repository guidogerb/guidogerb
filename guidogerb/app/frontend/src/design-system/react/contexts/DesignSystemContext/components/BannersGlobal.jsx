...
/** @typedef {import('design-system').DesignSystemContextBannerWithId} DesignSystemContextBannerWithId */
/**
 * @param {object} props
 * @param {DesignSystemContextBannerWithId[]} props.banners
...
export function BannersGlobal({ banners, bannerDuration, defaultClassName }) {
  ...
  const [zones, setZones] = useImmer(/** @type {Record<string, DesignSystemContextBannerWithId[]>} */({}));
  const currentOnClose = useCallback(
    /**
     * @param {import('react').MouseEvent | undefined} e
     * @param {DesignSystemContextBannerWithId} banner
     */
    (e, banner) => {
      if (banner.onClose) {
        banner.onClose(e);
  ...
  useEffect(() => {
    const draftZones = /** @type {Record<string, DesignSystemContextBannerWithId[]>} */ ({});
  ...
  return (
    <div
      className="design-system banner-global__wrapper"
      aria-live="polite"
    >
